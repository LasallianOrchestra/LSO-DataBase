(() => {
  ...
  function ensureBanner() {
    ...
    const content = isMembership()
      ...
      : isSecretary()
        ...
        : ['Staff Operations Access', 'Dashboard, member records, attendance monitoring, and Trainee/Probationary Duty Hours punch approval are available. Member, event, attendance, and manual Duty Hours editing remain disabled.'];
  ...
  function applyDutyControls(root) {
    ...
    const viewRoster = can('viewDutyRoster');
    qsa('.duty-management-only', root).forEach((node) => show(node, manage));
    qsa('.duty-roster-view-only', root).forEach((node) => show(node, viewRoster));
