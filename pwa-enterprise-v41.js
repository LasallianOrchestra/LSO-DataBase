(() => {
  ...
  const ACTIVE_CACHE_MARKER = 'self-attendance-v84';
  ...
  function updateConnection() {
    ...
    if (navigator.onLine) {
      // The cloud connector owns recovery and de-duplicates simultaneous reconnect requests.
      // Do not call loadSharedState here: V79 did that in parallel with the connector's own
      // online handler, creating competing requests and unstable status transitions.
      connectionTimer = window.setTimeout(async () => {
        banner('Connection restored. Verifying shared database…', 'online', 2400);
        try {
          if (window.LSOCloud?.recoverConnection) await window.LSOCloud.recoverConnection({ source: 'pwa-online', quiet: true });
          else await window.LSOCloud?.checkConnection?.({ quiet: true });
        } catch { /* the cloud connector publishes the authoritative status */ }
      }, 650);
    } else {
      ...
      registration.addEventListener('updatefound', () => {
        ...
        worker.addEventListener('statechange', () => {
          if (worker.state === 'installed' && navigator.serviceWorker.controller) {
            banner('A system operations update is ready. Applying it now…', 'online');
