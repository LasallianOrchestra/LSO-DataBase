(() => {
  ...
  function requiredMemberApprovers() {
    const field = el('dutySelfMemberApprovers');
    const value = String(field?.value || '').trim();
    if (field) {
      field.setCustomValidity(value ? '' : 'Enter the member name or names who approved or witnessed this duty.');
    }
    if (value) return value;
    if (field) {
      field.reportValidity();
      try { field.focus({ preventScroll: true }); } catch { field.focus(); }
      requestAnimationFrame(() => field.scrollIntoView({ block: 'center', inline: 'nearest', behavior: 'smooth' }));
    }
    window.LSOApp?.showToast?.('Member/s Approved is required before submitting Time In or Time Out.', true);
    return '';
  }
  ...
  async function timeInNow() {
    ...
    const memberApprovers = requiredMemberApprovers();
    if (!memberApprovers) return;
...
This will record the secure server time for the session that started on ${dateLabel(openEntry.date)} at ${formatClockTime(openEntry.timeIn)}. Time Out remains pending until an Administrator approves it separately.`
  ...
  function wireEvents() {
    ...
    const approverField = el('dutySelfMemberApprovers');
    approverField?.addEventListener('input', () => {
      approverField.setCustomValidity(approverField.value.trim() ? '' : 'Enter the member name or names who approved or witnessed this duty.');
    });
    ['dutySelfDescription', 'dutySelfMemberApprovers'].forEach((id) => {
      const field = el(id);
      field?.addEventListener('focus', () => document.body.classList.add('duty-mobile-input-active'));
      field?.addEventListener('blur', () => window.setTimeout(() => {
        if (!el('dutySelfEntryForm')?.contains(document.activeElement)) document.body.classList.remove('duty-mobile-input-active');
      }, 120));
    });
