(() => {
  ...
  // the Administrator. It must NOT depend on hard-coded role defaults: any configurable
  // role can be granted an operational action, so that same role must receive the write
  // column required by that action. This mirrors the V82 Supabase migration exactly.
  function deriveColumnsFromActions(actions = []) {
    const selected = new Set(normalizeList(actions));
    const columns = [];
    const add = (...items) => items.forEach((item) => { if (item && !columns.includes(item)) columns.push(item); });
    if (selected.has('manageMembers') || selected.has('generateContract')) add('members');
    if (selected.has('manageEvents') || selected.has('deleteEvents')) add('events');
    if (selected.has('saveDraftAttendance') || selected.has('finalizeAttendance') || selected.has('unlockAttendance') || selected.has('deleteEvents')) add('attendance');
    if (selected.has('finalizeAttendance') || selected.has('unlockAttendance')) add('settings');
    if (selected.has('manageDutyHours') || selected.has('manageDutyRequirements')) add('duty_hours');
    if (selected.has('editMonthlyReport') || selected.has('finalizeMonthlyReport') || selected.has('reopenMonthlyReport')) add('monthly_reports', 'monthly_reports_compat', 'settings');
    if (selected.has('writeActivityLog')) add('activity_log');
    return columns;
  }
  ...
  function applyServerPayload(payload, { emit = true } = {}) {
    ...
    roleRows(payload).forEach((row) => {
      ...
      const roleActions = normalizeList(row.actions).filter((action) =>
        actionKeys.includes(action) &&
        !PROTECTED_ACTIONS.has(action) &&
        (action !== 'selfDutyPunch' || roleName === ROLES.TRAINEE)
      );
      roleActions.forEach((action) => {
        next.actions[action] = normalizeList([...(next.actions[action] || []), roleName]);
      });

      // IMPORTANT: write columns are an internal consequence of operational permissions,
      // not a role-name entitlement. V81 fell back to each role's packaged column list
      // when older Supabase responses omitted `columns`; this fixed Membership defaults
      // but still broke custom grants to General Secretary, Staff Account, or Trainee.
      // Derive the exact required columns from the saved actions for EVERY configurable
      // role. This also rejects stale/over-broad server column rows after a permission is
      // revoked, keeping the browser and Supabase enforcement model aligned.
      next.columns[roleName] = deriveColumnsFromActions(roleActions);
  ...
  async function refreshFromServer({ quiet = true, force = false } = {}) {
    ...
    try {
      ...
      // Compatibility fallback keeps login usable while the V82 Supabase permission patch is not yet available.
  ...
  // The Interview grant is stored in synchronized Settings as a compatibility overlay on
  // databases whose permission RPC predates interviewView. Refresh immediately when that
  // shared Settings document changes so active accounts receive grants/revocations without
  // waiting for the 60-second permission poll.
  window.addEventListener('lso:cloud-state-changed', (event) => {
    ...
    if (!keys.includes('lso_system_settings_v2') || !window.LSOCurrentAccount) return;
    window.setTimeout(() => refreshFromServer({ quiet: true, force: true }).catch(() => undefined), 80);
  });
  ...
  window.LSORoleAccess = {
    ...
    deriveColumnsFromActions: (actions = []) => [...deriveColumnsFromActions(actions)],
    applyServerPayload,
    refreshFromServer,
    resetRuntime
  };
