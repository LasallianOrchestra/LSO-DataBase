(() => {
  ...
  const requiredForCompleteness = [
    'fullName', 'sex', 'studentNumber', 'college', 'course',
  ...
  function canManageMembers() {
    if (window.LSORoleAccess?.can) return window.LSORoleAccess.can('manageMembers');
    return (window.LSOAuth?.getActiveAccount?.() || window.LSOCurrentAccount)?.role === 'Administrator';
  }

  function requireMemberAccess(message = 'Administrator or Membership access is required to modify member records.') {
    if (canManageMembers()) return true;
    showToast(message, true);
    return false;
  }

  function requireAdminCompat(message = 'Administrator access is required.') {
  ...
  function printSelectedMemberRecord() {
    ...
    const fields = [
      ...
      ['DLSUD Outlook', member.outlook], ['College / Course', [member.college, member.course].filter(Boolean).join(' — ')],
      ['Year / Section', [member.yearLevel, member.section].filter(Boolean).join(' — ')], ['Trainee Period Start', toDateLabel(member.traineeStartDate)],
    ...
    popup.document.write(`<!doctype html><html><head><title>${safeText(member.fullName)} — Official Member Record</title><style>
      @page{size:A4 portrait;margin:0}*{box-sizing:border-box}body{font-family:Arial,Helvetica,sans-serif;color:#17211d;margin:0}.grid{display:grid;grid-template-columns:1fr 1fr;border:1px solid #8aa89b}.field{border-right:1px solid #8aa89b;border-bottom:1px solid #8aa89b}.field:nth-child(even){border-right:0}.notes{margin-top:16px;white-space:pre-wrap}.sign{display:grid;grid-template-columns:1fr 1fr}.foot{text-align:center}${window.LSOBrand?.printCss || ''}@media print{button{display:none}}</style></head><body>
  ...
  function openMemberModal(member = null) {
    ...
    const fieldMap = {
      fullName: 'fullName', birthdate: 'birthdate', age: 'age', sex: 'sex', homeAddress: 'homeAddress', studentNumber: 'studentNumber',
  ...
  function collectFormData() {
    ...
    const member = {
      ...
      // Legacy contact fields are preserved for existing records but are no longer collected, displayed, validated, or exported.
  ...
  function importCsv(file) {
    if (!requireMemberAccess('Administrator or Membership access is required to import member records.')) { if (el('csvImport')) el('csvImport').value = ''; return; }
  ...
  function restoreJson(file) {
    if (!requireAdminCompat('Administrator access is required to restore member records.')) { if (el('jsonRestore')) el('jsonRestore').value = ''; return; }
  ...
  function refreshAutomaticStages(showNotice = false) {
    ...
    if (before !== after && canManageMembers()) {
      window.LSOStorage.setItem(STORAGE_KEY, JSON.stringify(members));
      renderAll();
