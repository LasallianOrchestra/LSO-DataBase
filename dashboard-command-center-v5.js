(() => {
  ...
  function renderQuickActions() {
    const sets = {
      ...
      'Staff Account': [
        ['members', 'M', 'Member Directory', 'View and monitor member profiles'],
        ['attendance', '✓', 'Attendance Monitoring', 'View events, rates, and percentages'],
        ['duty-hours', '◷', 'Duty Approval Queue', 'Approve or reject Time In and Time Out']
      ]
    };
    const actions = sets[role()] || sets['Staff Account'];
    const heading = isAdmin() ? 'Administrative tools' : role() === 'Staff Account' ? 'Staff monitoring and approval tools' : `${role()} tools`;
