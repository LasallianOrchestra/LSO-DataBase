(() => {
  ...
  function renderPermissionMatrix() {
    ...
    const roles = Object.values(CORE.ROLES || {}); const manifest = CORE.PERMISSIONS || { actions:{}, views:{} };
  ...
  function exportPermissionMatrix() {
    const roles=Object.values(CORE.ROLES||{});const manifest=CORE.PERMISSIONS||{actions:{},views:{}};const rows=[['Category','Permission',...roles]];
    const viewLabels={dashboardView:'Dashboard',membersView:'Members',lookupView:'Member Lookup',contractView:'Contract',monthlyReportView:'Monthly Report',attendanceView:'Attendance',dutyHoursView:'Duty Hours',alertsView:'Action Center',accountsView:'Accounts',systemHealthView:'System Health',dataView:'Data & Recovery'};
  ...
  function printDutyCertification() {
    ...
    popup.document.write(`<!doctype html><html><head><title>Duty Hours Certification</title><style>@page{size:A4 portrait;margin:18mm}body{font-family:Arial;color:#17211d}.header{text-align:center;border-bottom:4px solid #146c43;padding-bottom:18px}.org{color:#146c43;font-weight:800;letter-spacing:.12em;text-transform:uppercase}.title{font-size:28px;margin:12px}.card{margin:28px 0;border:1px solid #d8e2dc;border-radius:16px;padding:22px}.grid{display:grid;grid-template-columns:1fr 1fr;gap:14px}.grid div{background:#f6f8f5;padding:13px;border-radius:10px}.grid span{display:block;font-size:11px;text-transform:uppercase;color:#5f6b65}.grid strong{display:block;font-size:18px;margin-top:5px}.statement{line-height:1.8;text-align:justify}.sign{display:grid;grid-template-columns:1fr 1fr;gap:70px;margin-top:80px;text-align:center}.sign div{border-top:1px solid #17211d;padding-top:8px}</style></head><body><div class="header"><div class="org">Lasallian Symphony Orchestra</div><div class="title">Duty Hours Certification</div><div>${safeText(semester)} • ${safeText(period)}</div></div><div class="card"><p class="statement">This certifies that <strong>${safeText(member.fullName)}</strong> (${safeText(member.membershipId || member.studentNumber || 'member record')}) has a verified Duty Hours record for the period stated above. Only approved entries are included in the credited total.</p><div class="grid"><div><span>Required</span><strong>${safeText(durationLabel(calc.committed))}</strong></div><div><span>Credited</span><strong>${safeText(durationLabel(calc.credited))}</strong></div><div><span>Remaining</span><strong>${safeText(durationLabel(calc.remaining))}</strong></div><div><span>Completion</span><strong>${safeText(calc.percent)}%</strong></div></div></div><div class="sign"><div>Member Signature</div><div>Authorized Officer</div></div><script>window.onload=()=>window.print()<\/script></body></html>`); popup.document.close();
  }
  ...
  function enterpriseNotifications() {
    const notifications=[];const accountRole=role();
    if(['Administrator','Membership'].includes(accountRole)){
      const data=dutyData();(data.entries||[]).forEach((entry)=>{
        if(entry.timeInApprovalStatus==='Pending')notifications.push({id:`enterprise-duty-in:${entry.id}`,category:'Duty Approval',severity:'high',title:'Time In requires approval',detail:'Open the separate Duty Hours approval queue.',timestamp:entry.timeInRequestedAt||entry.createdAt||'',actionType:'duty-hours',targetId:entry.id,icon:'IN'});
        if(entry.timeOutApprovalStatus==='Pending')notifications.push({id:`enterprise-duty-out:${entry.id}`,category:'Duty Approval',severity:'high',title:'Time Out requires approval',detail:'Review the completed Duty Hours punch.',timestamp:entry.timeOutRequestedAt||entry.submittedAt||'',actionType:'duty-hours',targetId:entry.id,icon:'OUT'});
      });
      const reports=monthlyState().reports||{};Object.values(reports).forEach((report)=>{if(report?.workflowStatus!=='Finalized'&&report?.month){notifications.push({id:`enterprise-monthly:${report.month}`,category:'Monthly Report',severity:'medium',title:`${report.month} Monthly Report remains Draft`,detail:'Complete the filing and finalize it when verified.',timestamp:report.updatedAt||'',actionType:'monthly-report',targetId:report.month,icon:'PDF'});}});
    }
