(() => {
  ...
  function canManageDutyHours() {
    return window.LSORoleAccess?.can?.('manageDutyHours') ?? ['Administrator', 'Membership'].includes(currentAccount()?.role);
  }

  function canReviewDutyPunches() {
    return window.LSORoleAccess?.can?.('reviewDutyPunches') ?? ['Administrator', 'Membership', 'Staff Account'].includes(currentAccount()?.role);
  }

  // Kept as an internal compatibility alias for management-only controls.
  function isAdmin() { return canManageDutyHours(); }
  ...
  function defaultData() {
    return { version: 8, commitments: {}, entries: [], archiveExclusions: {} };
  }
  ...
  function normalizeData(raw) {
    ...
    return {
      ...
      archiveExclusions: data.archiveExclusions && typeof data.archiveExclusions === 'object' && !Array.isArray(data.archiveExclusions) ? { ...data.archiveExclusions } : {}
    };
  }
  ...
  function ensureDataMigration() {
    ...
    if (raw?.version === 8 || !isAdmin()) return;
  ...
  function archiveExclusionKey(memberId, period) {
    return `${String(memberId || '')}:${normalizePeriod(period)}`;
  }

  function archiveIsExcluded(data, memberId, period) {
    return Boolean(data.archiveExclusions?.[archiveExclusionKey(memberId, period)]);
  }

  function currentDutyMember(member, period) {
    if (!member || member.periodGroup !== normalizePeriod(period)) return false;
    const status = String(member.memberStatus || member.status || member.currentStatus || '').trim().toLowerCase();
    return !['nonactive', 'inactive', 'loa', 'leave of absence', 'archived', 'removed'].includes(status);
  }
  ...
  function periodLifecycle(member, period, data) {
    ...
    const excludedFromArchive = archiveIsExcluded(data, member.id, normalizedPeriod);
    ...
    if (normalizedPeriod === 'Trainee Period') {
      ...
      const archived = eligible && !isCurrent && (progressed || hasData) && !excludedFromArchive;
      return { eligible, active: isCurrent, archived, skipped: false, excludedFromArchive, label: isCurrent ? 'Active' : archived ? 'Archived' : excludedFromArchive ? 'Removed from archive' : 'Not started' };
    }

    const eligible = isCurrent || isMember || Boolean(member.probationaryStartDate) || skippedProbationary || hasData;
    const archived = eligible && !isCurrent && (isMember || skippedProbationary || hasData) && !excludedFromArchive;
    return {
      eligible,
      active: isCurrent,
      archived,
      ...
      excludedFromArchive,
      label: isCurrent ? 'Active' : skippedProbationary && archived ? 'Skipped • Archived' : archived ? 'Archived' : excludedFromArchive ? 'Removed from archive' : 'Not started'
    };
  }

  function rosterMembers(data, period, applySearch = true, mode = recordMode) {
    const query = applySearch ? searchTerm.trim().toLowerCase() : '';
    const requestedMode = isStaffAccount() ? 'Active' : mode;
    return getMembers()
      .filter((member) => {
        if (isStaffAccount()) return currentDutyMember(member, period);
        const lifecycle = periodLifecycle(member, period, data);
        if (!lifecycle.eligible) return false;
        if (requestedMode === 'Active') return lifecycle.active;
        if (requestedMode === 'Archive') return lifecycle.archived;
  ...
  function rosterButton(member, period, data) {
    ...
    const lifecycle = periodLifecycle(member, period, data);
    const removeAction = recordMode === 'Archive' && canManageDutyHours()
      ? `<button class="duty-archive-delete-button" data-duty-archive-delete="${safeText(member.id)}" data-duty-archive-period="${safeText(period)}" type="button" aria-label="Remove ${safeText(member.fullName)} from ${safeText(period)} archive">Remove from Archive</button>`
      : '';
    return `<div class="duty-roster-person-shell${selected ? ' active' : ''}">
      <button class="duty-roster-person${selected ? ' active' : ''}${lifecycle.archived ? ' archived-record' : ''}${lifecycle.skipped ? ' skipped-record' : ''}" data-duty-member="${safeText(member.id)}" data-duty-roster-period="${safeText(period)}" type="button">
  ...
  function deleteArchivedPeriod(memberId, period) {
    if (!canManageDutyHours() || recordMode !== 'Archive') return;
    const normalizedPeriod = normalizePeriod(period);
    const member = getMembers().find((item) => item.id === memberId);
    if (!member) return;
    const confirmed = window.confirm(`Remove ${member.fullName} from the ${normalizedPeriod} archive?\n\nThis removes only that archived Duty Hours period, including its archived commitment and ledger entries. The member profile and the other Duty Hours period remain unchanged.`);
    if (!confirmed) return;
    const reason = window.prompt('Enter the reason for removing this archived Duty Hours record:');
    if (reason === null) return;
    if (reason.trim().length < 3) return window.LSOApp?.showToast?.('A clear removal reason is required.', true);
    const data = loadData();
    data.entries = data.entries.filter((entry) => !(entry.memberId === memberId && entry.period === normalizedPeriod));
    const key = periodKey(normalizedPeriod);
    if (data.commitments?.[memberId]) {
      SEMESTERS.forEach((semester) => {
        if (data.commitments[memberId][semester]) data.commitments[memberId][semester][key] = 0;
      });
    }
    data.archiveExclusions ||= {};
    data.archiveExclusions[archiveExclusionKey(memberId, normalizedPeriod)] = {
      memberId,
      period: normalizedPeriod,
      removedAt: new Date().toISOString(),
      removedBy: currentAccount()?.displayName || currentAccount()?.username || 'Administrator',
      reason: reason.trim()
    };
    if (selectedMemberId === memberId && selectedPeriod === normalizedPeriod) {
      selectedMemberId = '';
      selectedPeriod = 'Trainee Period';
    }
    persistData(data, { action: 'Removed archived Duty Hours period', details: `${member.fullName} • ${normalizedPeriod} • ${reason.trim()}` });
    window.LSOApp?.showToast?.(`${member.fullName} was removed from the ${normalizedPeriod} archive.`);
    renderAll();
  }

  function renderRosters() {
    if (isStaffAccount()) recordMode = 'Active';
    const data = loadData();
    document.body?.classList.toggle('duty-staff-view', isStaffAccount());
    const monitorPanel = el('dutyRosterMonitorPanel');
    if (monitorPanel) monitorPanel.classList.toggle('hidden', isTraineeAccount());
  ...
  function renderTodaySessions(data, member) {
    ...
    container.innerHTML = sessions.length ? sessions.map((entry) => {
      ...
      return `<article class="duty-today-session ${safeText(display.className)}${isOpen ? ' is-open-request' : ''}" data-duty-session-id="${safeText(entry.id)}">
        ...
        ${isOpen ? '<div class="duty-open-request-notice"><strong>This request is already saved.</strong><span>Submit Time Out when your duty ends. The Administrator, Membership, or Staff reviews Time In and Time Out separately.</span></div>' : ''}
    ...
    }).join('') : '<div class="empty-state compact-empty duty-session-empty"><h4>No duty request has been submitted today</h4><p>Press Submit Time In once. The saved request will immediately appear here while it waits for Administrator, Membership, or Staff review.</p></div>';
  }
  ...
  function renderSelfService() {
    ...
    if (openEntry) {
      if (heading) heading.textContent = openInStatus === 'Approved' ? 'Time In approved — submit Time Out when finished' : 'Time In awaiting Administrator, Membership, or Staff approval';
      ...
      if (helper) helper.textContent = 'Pressing the button creates a Time In request. It becomes official only after Administrator, Membership, or Staff approval.';
    }
  ...
  function renderApprovalQueue() {
    ...
    panel.classList.toggle('hidden', !canReviewDutyPunches());
    if (!canReviewDutyPunches()) return;
    const data = loadData();
    ...
    }).join('') : '<div class="empty-state compact-empty duty-review-empty"><h4>No pending punch requests</h4><p>Separate Time In and Time Out requests will appear here for Administrator, Membership, or Staff review.</p></div>';
  }
  ...
  function renderLedger(data, member) {
    ...
    body.innerHTML = entries.length ? entries.map((entry) => {
      ...
      const countedEffect = entry.approvalStatus === 'Approved' ? effect : entry.approvalStatus === 'Active' ? 'Time In approved; Time Out not yet approved' : entry.approvalStatus === 'Pending' ? 'No punch applied until Administrator, Membership, or Staff approval' : 'Rejected — not credited';
  ...
  async function timeInNow() {
    const memberApprovers = el('dutySelfMemberApprovers')?.value.trim() || '';
    if (!memberApprovers) {
      el('dutySelfMemberApprovers')?.focus();
      return window.LSOApp?.showToast?.('Member/s Approved is required before submitting Time In.', true);
    }
    ...
    try {
      ...
      if (openEntry) {
        ...
        const message = inStatus === 'Pending'
          ? `Your Time In request for ${dateLabel(openEntry.date)} at ${formatClockTime(openEntry.timeIn)} is already saved and waiting for Administrator, Membership, or Staff approval.`
      ...
      window.LSOApp?.showToast?.(`Time In at ${formatClockTime(saved.timeIn)} was submitted and is waiting for Administrator, Membership, or Staff approval.`);
  ...
  async function timeOutNow() {
    ...
    if (!memberApprovers) {
      el('dutySelfMemberApprovers')?.focus();
      return window.LSOApp?.showToast?.('Member/s Approved is required before submitting Time Out.', true);
    }
    if (!isTraineeAccount()) return;
    const member = linkedMember();
...
This will record the secure server time for the session that started on ${dateLabel(openEntry.date)} at ${formatClockTime(openEntry.timeIn)}. Time Out remains pending until an Administrator approves it separately.`
      ...
      renderAll();
      window.LSOApp?.showToast?.('Time Out was submitted for separate Administrator, Membership, or Staff approval. No minutes are credited until both punches are approved.');
    } catch (error) {
  ...
  async function reviewDutyPunch(entryId, punchType, decision) {
    if (!canReviewDutyPunches() || !entryId || !['TimeIn', 'TimeOut'].includes(punchType) || !['Approved', 'Rejected'].includes(decision)) return;
    const data = loadData();
  ...
  function printIndividual() {
    ...
    const html = `<!doctype html><html><head><title>${safeText(member.fullName)} — Duty Hours</title><style>${printStyles('portrait')}</style></head><body class="lso-print-portrait">
  ...
  function memberContactDetails(member) {
    const details = [member.outlook].filter(Boolean);
  ...
  function printPeriodMembers(period) {
    ...
    const html = `<!doctype html><html><head><title>Current ${safeText(normalizedPeriod)} Members — Duty Hours</title><style>${printStyles('portrait')}
  ...
  function printMonthlyPeriodReport(period) {
    ...
    const html = `<!doctype html><html><head><title>${safeText(monthName)} ${safeText(normalizedPeriod)} Duty Hours</title><style>${printStyles('portrait')}
      body{font-size:9px}.report-note{padding:9px 11px;border:1px solid #d8e6df;border-radius:7px;background:#f7fbf9;margin:10px 0 16px;line-height:1.45}.monthly-roster{font-size:5.8px}.monthly-roster th{font-size:5.2px}.monthly-roster td{padding:3px}.muted{color:#687c74;font-size:7px}.section-title{display:flex;justify-content:space-between;align-items:end;margin:20px 0 7px}.section-title h2{margin:0;font-size:15px;color:#0b4c3a}.page-break{break-before:page;page-break-before:always}
      </style></head><body class="lso-print-portrait">
  ...
  function printOverall() {
    ...
    const html = `<!doctype html><html><head><title>${safeText(activeSemester)} Duty Hours</title><style>${printStyles('portrait')}</style></head><body class="lso-print-portrait">
  ...
  function renderAll() {
    if (isStaffAccount()) {
      recordMode = 'Active';
      const staffData = loadData();
      const allowedIds = new Set(PERIODS.flatMap((period) => rosterMembers(staffData, period, false, 'Active').map((member) => `${member.id}:${period}`)));
      if (selectedMemberId && !allowedIds.has(`${selectedMemberId}:${selectedPeriod}`)) {
        selectedMemberId = '';
        selectedPeriod = 'Trainee Period';
      }
    }
  ...
  function wireEvents() {
    ...
    el('dutyRecordModeToggle')?.addEventListener('click', (event) => {
      ...
      if (!button) return;
      if (isStaffAccount() && button.dataset.dutyRecordMode === 'Archive') {
        recordMode = 'Active';
        return window.LSOApp?.showToast?.('Staff accounts can monitor current Trainee and Probationary members only.', true);
      }
    ...
    ['dutyTraineeRoster', 'dutyProbationaryRoster'].forEach((id) => el(id)?.addEventListener('click', (event) => {
      const archiveDelete = event.target.closest('[data-duty-archive-delete]');
      if (archiveDelete) {
        event.preventDefault();
        deleteArchivedPeriod(archiveDelete.dataset.dutyArchiveDelete, archiveDelete.dataset.dutyArchivePeriod);
        return;
      }
