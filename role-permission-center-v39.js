(() => {
  ...
  async function load({ quiet = false, force = false } = {}) {
    ...
    loadPromise = (async () => {
      ...
      try {
        if (!window.LSOCloud?.getRolePermissionCenter) throw new Error('The permission connector is unavailable. Upload the complete V82 package and apply LSO_V82_ROLE_SYNC_SUPABASE_PATCH.sql in Supabase.');
  ...
  async function save() {
    ...
    try {
      ...
      const savedRow = (nextPayload?.roles || []).find((row) => String(row?.roleName || row?.role || '') === activeRole);
      if (!savedRow) throw new Error('The shared permission configuration did not return the role that was saved. No successful save was reported.');
      const sameSet = (expected, actual) => {
        const a = [...new Set(expected || [])].sort();
        const b = [...new Set(actual || [])].sort();
        return a.length === b.length && a.every((item, index) => item === b[index]);
      };
      const problems = [];
      if (!sameSet(views, savedRow.views)) problems.push('modules');
      if (!sameSet(actions, savedRow.actions)) problems.push('actions');
      if (!sameSet(attendanceGroups, savedRow.attendanceGroups)) problems.push('attendance calendars');
      if (String(savedRow.landingView || '') !== landingView) problems.push('landing page');
      if (problems.length) {
        throw new Error(`The shared permission configuration did not preserve the selected ${problems.join(', ')}. No successful save was reported.`);
      }
      applyPayload(nextPayload);
      const expectedColumns = window.LSORoleAccess?.deriveColumnsFromActions?.(actions) || [];
      const runtimeColumns = window.LSORoleAccess?.permissionManifest?.()?.columns?.[activeRole] || [];
      if (!sameSet(expectedColumns, runtimeColumns)) {
        throw new Error('Operational permissions were saved, but their database write access did not reconcile correctly. The role was not reported as fully synchronized.');
      }
      renderMatrix();
      ...
      status(`${activeRole} permissions were saved and verified in the shared permission configuration. Active users receive the update automatically.`, 'success');
