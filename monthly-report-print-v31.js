(() => {
  ...
  async function buildPdfBytes() {
    ...
    if (!window.LSO_MONTHLY_REPORT_TEMPLATE_BASE64) throw new Error('The default official LSO document template is missing. Upload monthly-report-template-data.js and lso-official-template.pdf to GitHub.');
    ...
    const colors = {
      ...
      green: rgb(0.043, 0.239, 0.18),
      emerald: rgb(0.078, 0.424, 0.263),
      officer: rgb(0.91, 0.95, 0.92),
      highlightGreen: rgb(0.91, 0.95, 0.92),
      yellow: rgb(0.043, 0.239, 0.18),
      gold: rgb(0.831, 0.627, 0.09),
      goldSoft: rgb(1, 0.957, 0.80),
      gray: rgb(0.945, 0.965, 0.952),
      white: rgb(1, 1, 1),
      line: rgb(0.16, 0.31, 0.25)
    };
    ...
    function drawSignatoryBlock(page, x, y, width, label, name, title) {
      const centeredX = (font, value, size) => x + Math.max(0, (width - textWidth(font, value, size)) / 2);
      page.drawText(label, { x: centeredX(timesBold, label, 9.5), y, size: 9.5, font: timesBold, color: colors.green });
      page.drawLine({ start: { x, y: y - 36 }, end: { x: x + width, y: y - 36 }, thickness: 0.9, color: colors.green });
      const displayName = String(name || ' ').trim() || ' ';
      page.drawText(displayName, { x: centeredX(timesBold, displayName, 9), y: y - 51, size: 9, font: timesBold, color: colors.ink });
      const displayTitle = organizationTitle(title, label === 'Prepared By:' ? 'EVP for Membership' : 'President');
      const titleLines = wrap(timesItalic, displayTitle, 8.5, width - 8);
      titleLines.slice(0, 2).forEach((line, index) => page.drawText(line, {
        x: centeredX(timesItalic, line, 8.5), y: y - 72 - index * 11, size: 8.5, font: timesItalic, color: colors.ink
      }));
    }
    ...
    function drawTableHeader(page, columns, x, y, options = {}) {
      ...
      columns.forEach((column, index) => {
        const fill = fills[index] || colors.green;
        const headerTextColor = textColors[index] || (fill === colors.white || fill === colors.gray || fill === colors.highlightGreen ? colors.ink : colors.white);
        page.drawRectangle({
          x: cursor, y: y - height, width: column.width, height,
          color: fill,
          borderColor: colors.line, borderWidth: 0.9
        });
        const labelLines = wrap(helveticaBold, column.label, size, column.width - 8).slice(0, 2);
        const lineHeight = size + 1.5;
        const totalHeight = labelLines.length * lineHeight;
        const firstBaseline = y - (height - totalHeight) / 2 - size;
        labelLines.forEach((line, lineIndex) => {
          const tx = cursor + Math.max(4, (column.width - textWidth(helveticaBold, line, size)) / 2);
          page.drawText(line, { x: tx, y: firstBaseline - lineIndex * lineHeight, size, font: helveticaBold, color: headerTextColor });
        });
        cursor += column.width;
      });
      return y - height;
    ...
    function drawTableRow(page, columns, row, x, y, options = {}) {
      ...
      columns.forEach((column, index) => {
        ...
        const baseFill = options.total ? colors.goldSoft : (options.background || (options.stripe ? colors.gray : colors.white));
        const fill = fills[index] || baseFill;
        page.drawRectangle({
          x: cursor, y: y - height, width: column.width, height,
          color: fill, borderColor: colors.line, borderWidth: options.total ? 0.9 : 0.65
        });
        ...
        lines.forEach((line, lineIndex) => {
          const align = column.align || (index === 0 ? 'left' : 'center');
    ...
    async function drawPaginatedTable({
      ...
      for (let index = 0; index < tableRows.length; index += 1) {
        ...
        y = drawTableRow(page, columns, row, bounds.left, y, { size: fontSize, stripe: index % 2 === 1, ...style, height });
      }
    ...
    {
      const page = await templatePage();
      drawCentered(page, 'SUMMARY OF MANPOWER COUNT', 622, 12.5, timesBold, colors.green);
      ...
      drawCentered(page, 'Table 1. According to Gender', 542, 9.5, timesBold, colors.green);
      ...
      drawCentered(page, 'Table 2. According to the Year Level', y - 38, 9.5, timesBold, colors.green);
    ...
    await drawPaginatedTable({
      ...
      drawHeading(page, continued) {
        if (continued) return 632;
        drawCentered(page, 'Table 3. Summary of Members on Leave', 622, 10, timesBold, colors.green);
        return 600;
      },
      rowStyle() {
    ...
    await drawPaginatedTable({
      ...
      drawHeading(page, continued) {
        if (continued) return 632;
        drawCentered(page, 'Table 4. Summary of Members on "ON THE JOB TRAINING" (OJT)', 622, 9.6, timesBold, colors.green);
        return 600;
      },
    ...
    await drawPaginatedTable({
      ...
      drawHeading(page, continued) {
        if (continued) return 632;
        drawCentered(page, 'Table 5. Summary of Trainees / Probationary Members', 622, 9.4, timesBold, colors.green);
        const subtitle = `Monthly filing as of ${filingDate}. Original Entry Dates are entered by the report preparer.`;
        drawCentered(page, subtitle, 601, 7.2, times, colors.ink);
        return 584;
      },
      rowStyle() {
    ...
    await drawPaginatedTable({
      ...
      drawHeading(page, continued) {
        if (continued) return 632;
        drawCentered(page, 'Table 6. Summary of Quitted/Removed Trainees', 622, 10, timesBold, colors.green);
        return 600;
      },
    ...
    await drawPaginatedTable({
      ...
      drawHeading(page, continued) {
        if (continued) return 632;
        drawCentered(page, 'Table 7. Summary of Remaining Trainees', 622, 10, timesBold, colors.green);
        return 600;
      },
      rowStyle() { return { minHeight: 24 }; }
    });
